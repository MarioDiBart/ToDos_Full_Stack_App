...
const Product = class Product {
    constructor(input){
        ...
        this.price = parseFloat(this.array[1]);
        this.availability = this.array[2];
    }
    ...
    static halfOff = (products) => {
        ...
        fakeArray.push(new Product(products[i].input));
        fakeArray[i].price *= .50;
        }
        return fakeArray;
    }
    ...
    for(var j = 0; j < products.length;j++){
        var available;
        if(products[j].availability == 'In Stock'){
            available = 'Yes';
        }
        else {
            available = 'No';
        }
        console.log("Product: " + products[j].name + ", Cost: $" + (products[j].price.toFixed(2)+ ", Availability: " + available));
    }
 }
}
// ----------------------------------
